...
export function AuditLogTable({ logs, isLoading, activeDropdown, setActiveDropdown, onDelete }: AuditLogTableProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 shadow-sm w-full rounded-[4px] overflow-hidden min-h-[400px]">
      <div className="bg-[#175FFF] flex items-center h-[46px] px-6 text-white font-mono text-[12px] font-bold tracking-wider">
      ...
      {isLoading ? (
        ...
        <div className="divide-y divide-gray-100 bg-white pb-4">
          {logs.map((log) => (
            <div key={log.id} className="flex items-start px-6 py-4 hover:bg-white transition-colors relative">
              ...
              <div className="w-[5%] flex justify-end">
                ...
                {activeDropdown === log.id && (
                  <div className="absolute right-12 top-10 bg-white border border-gray-200 shadow-lg rounded-[4px] w-[140px] z-10 py-1 overflow-hidden font-mono">
                    ...
                    <button onClick={() => onDelete(log.id)} className="w-full flex items-center px-4 py-2.5 text-[12px] font-bold text-[#F04438] hover:bg-red-50 transition-colors">
